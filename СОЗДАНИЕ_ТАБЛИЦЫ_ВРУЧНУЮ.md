# 🛠️ Создание таблицы ContactMessages вручную

Если миграции не работают, можно создать таблицу вручную через SQL.

## SQL скрипт для SQL Server

Выполните этот SQL скрипт в вашей базе данных `CloudCityDB`:

```sql
USE CloudCityDB;
GO

-- Создание таблицы ContactMessages
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ContactMessages]') AND type in (N'U'))
BEGIN
    CREATE TABLE [dbo].[ContactMessages] (
        [Id] INT NOT NULL IDENTITY(1,1),
        [Name] NVARCHAR(200) NOT NULL,
        [Email] NVARCHAR(200) NOT NULL,
        [Phone] NVARCHAR(50) NULL,
        [Subject] NVARCHAR(200) NULL,
        [ServiceType] NVARCHAR(100) NULL,
        [Message] NVARCHAR(MAX) NOT NULL,
        [SourcePage] NVARCHAR(50) NOT NULL,
        [CreatedAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
        [IsRead] BIT NOT NULL DEFAULT 0,
        [ReadAt] DATETIME2 NULL,
        CONSTRAINT [PK_ContactMessages] PRIMARY KEY CLUSTERED ([Id] ASC)
    );
    
    PRINT 'Таблица ContactMessages создана успешно';
END
ELSE
BEGIN
    PRINT 'Таблица ContactMessages уже существует';
END
GO
```

## Как выполнить

### Вариант 1: Через sqlcmd

```bash
sqlcmd -S 10.151.10.8 -U sa -P ваш_пароль -d CloudCityDB -i create_contact_messages.sql
```

Или в одну строку:

```bash
sqlcmd -S 10.151.10.8 -U sa -P ваш_пароль -d CloudCityDB -Q "CREATE TABLE [dbo].[ContactMessages] ([Id] INT NOT NULL IDENTITY(1,1), [Name] NVARCHAR(200) NOT NULL, [Email] NVARCHAR(200) NOT NULL, [Phone] NVARCHAR(50) NULL, [Subject] NVARCHAR(200) NULL, [ServiceType] NVARCHAR(100) NULL, [Message] NVARCHAR(MAX) NOT NULL, [SourcePage] NVARCHAR(50) NOT NULL, [CreatedAt] DATETIME2 NOT NULL DEFAULT GETUTCDATE(), [IsRead] BIT NOT NULL DEFAULT 0, [ReadAt] DATETIME2 NULL, CONSTRAINT [PK_ContactMessages] PRIMARY KEY CLUSTERED ([Id] ASC));"
```

### Вариант 2: Через SQL Server Management Studio (SSMS)

1. Подключитесь к серверу `10.151.10.8`
2. Выберите базу данных `CloudCityDB`
3. Откройте "New Query"
4. Вставьте SQL скрипт выше
5. Выполните (F5)

### Вариант 3: Через Azure Data Studio или другой клиент

1. Подключитесь к серверу
2. Выберите базу данных
3. Выполните SQL скрипт

## После создания таблицы

1. Удалите миграцию `AddContactMessages` (если она была создана):
   ```bash
   cd /home/siteadmin/cloudcity/CloudCityCenter
   dotnet ef migrations remove --project CloudCityCenter
   ```

2. Создайте пустую миграцию, которая будет знать о существующей таблице:
   ```bash
   export ConnectionStrings__DefaultConnection="Server=10.151.10.8;Database=CloudCityDB;User Id=sa;Password=ваш_пароль;TrustServerCertificate=True"
   dotnet ef migrations add AddContactMessages --project CloudCityCenter
   ```

3. Откройте созданный файл миграции и удалите из него команды `CREATE TABLE` (таблица уже существует)

4. Перезапустите сервис:
   ```bash
   sudo systemctl restart cloudcity
   ```

## Проверка

```bash
# Проверьте, что таблица создана
sqlcmd -S 10.151.10.8 -U sa -P ваш_пароль -d CloudCityDB -Q "SELECT COUNT(*) FROM ContactMessages"
```

Должно вернуть `0` (таблица пустая, но существует).

---

**После создания таблицы страница `/Admin/Messages` должна работать!**


