# Пошаговое решение всех проблем

## Проблема 1: `dotnet ef` не найден

**Решение:**
```bash
# Установите EF Core tools
dotnet tool install --global dotnet-ef

# Добавьте в PATH (если нужно)
export PATH="$PATH:$HOME/.dotnet/tools"

# Проверьте
dotnet ef --version
```

## Проблема 2: "Invalid column name 'slug'"

**Причина:** Таблицы созданы без колонки `slug` или миграция не применилась.

**Решение:** Удалите все таблицы и создайте заново через правильную миграцию.

## Проблема 3: Нельзя удалить таблицу Products

**Причина:** Есть внешние ключи от других таблиц.

**Решение:** Удаляйте таблицы в правильном порядке (сначала дочерние).

---

## ✅ Полное решение (выполните по порядку)

### Шаг 1: Установите EF Core Tools

```bash
cd /home/siteadmin/cloudcity/CloudCityCenter
dotnet tool install --global dotnet-ef
export PATH="$PATH:$HOME/.dotnet/tools"
dotnet ef --version
```

### Шаг 2: Удалите все таблицы в SQL Server

Откройте SQL Server Management Studio и выполните скрипт `drop_all_tables.sql`:

```sql
USE CloudCityDB;
GO

-- Удаляем в правильном порядке
IF OBJECT_ID('dbo.ProductFeatures', 'U') IS NOT NULL DROP TABLE dbo.ProductFeatures;
IF OBJECT_ID('dbo.ProductVariants', 'U') IS NOT NULL DROP TABLE dbo.ProductVariants;
IF OBJECT_ID('dbo.OrderItems', 'U') IS NOT NULL DROP TABLE dbo.OrderItems;
IF OBJECT_ID('dbo.Orders', 'U') IS NOT NULL DROP TABLE dbo.Orders;
IF OBJECT_ID('dbo.Products', 'U') IS NOT NULL DROP TABLE dbo.Products;
IF OBJECT_ID('dbo.Servers', 'U') IS NOT NULL DROP TABLE dbo.Servers;
IF OBJECT_ID('dbo.AspNetUserClaims', 'U') IS NOT NULL DROP TABLE dbo.AspNetUserClaims;
IF OBJECT_ID('dbo.AspNetUserLogins', 'U') IS NOT NULL DROP TABLE dbo.AspNetUserLogins;
IF OBJECT_ID('dbo.AspNetUserRoles', 'U') IS NOT NULL DROP TABLE dbo.AspNetUserRoles;
IF OBJECT_ID('dbo.AspNetUserTokens', 'U') IS NOT NULL DROP TABLE dbo.AspNetUserTokens;
IF OBJECT_ID('dbo.AspNetRoleClaims', 'U') IS NOT NULL DROP TABLE dbo.AspNetRoleClaims;
IF OBJECT_ID('dbo.AspNetRoles', 'U') IS NOT NULL DROP TABLE dbo.AspNetRoles;
IF OBJECT_ID('dbo.AspNetUsers', 'U') IS NOT NULL DROP TABLE dbo.AspNetUsers;
IF OBJECT_ID('dbo.__EFMigrationsHistory', 'U') IS NOT NULL DROP TABLE dbo.__EFMigrationsHistory;
GO
```

### Шаг 3: Удалите старые миграции и создайте новые

```bash
cd /home/siteadmin/cloudcity/CloudCityCenter

# Установите окружение
export ASPNETCORE_ENVIRONMENT=Production

# Установите строку подключения
export ConnectionStrings__DefaultConnection="Server=10.151.10.8;Database=CloudCityDB;User Id=sa;Password=ВАШ_ПАРОЛЬ;TrustServerCertificate=True;MultipleActiveResultSets=true"

# Удалите старые миграции
rm -rf Migrations

# Создайте новую миграцию для SQL Server
dotnet ef migrations add InitialCreateForSqlServer

# Примените миграцию
dotnet ef database update
```

### Шаг 4: Загрузите данные

```bash
# Загрузите товары и услуги
dotnet run -- --migrate-data
```

### Шаг 5: Проверьте результат

В SQL Server Management Studio:

```sql
USE CloudCityDB;
SELECT COUNT(*) as ProductsCount FROM Products;  -- Должно быть 16
SELECT COUNT(*) as VariantsCount FROM ProductVariants;  -- Должно быть 16
SELECT COUNT(*) as FeaturesCount FROM ProductFeatures;  -- Должно быть ~80-100

-- Проверьте структуру таблицы Products
SELECT COLUMN_NAME, DATA_TYPE 
FROM INFORMATION_SCHEMA.COLUMNS 
WHERE TABLE_NAME = 'Products'
ORDER BY ORDINAL_POSITION;
```

Должна быть колонка `Slug` (с большой буквы в SQL Server).

---

## 🚀 Быстрый способ (автоматический скрипт)

Используйте готовый скрипт:

```bash
cd /home/siteadmin/cloudcity/CloudCityCenter
chmod +x setup_and_migrate.sh

# Установите строку подключения
export ConnectionStrings__DefaultConnection="Server=10.151.10.8;Database=CloudCityDB;User Id=sa;Password=ВАШ_ПАРОЛЬ;TrustServerCertificate=True;MultipleActiveResultSets=true"

# Запустите скрипт
./setup_and_migrate.sh
```

**ВАЖНО:** Перед запуском скрипта удалите таблицы через SQL скрипт `drop_all_tables.sql`!

---

## 🔍 Если что-то пошло не так

1. **Проверьте строку подключения:**
   ```bash
   echo $ConnectionStrings__DefaultConnection
   ```

2. **Проверьте окружение:**
   ```bash
   echo $ASPNETCORE_ENVIRONMENT
   ```
   Должно быть `Production`

3. **Проверьте доступность SQL Server:**
   ```bash
   telnet 10.151.10.8 1433
   ```

4. **Проверьте логи:**
   ```bash
   dotnet run -- --migrate-data 2>&1 | tee migration.log
   ```




