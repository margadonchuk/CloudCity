# ✅ Правильный запуск приложения

## Проблема

Приложение запускается в режиме **Development** и использует порт **5076** вместо **5000**.

## ✅ Решение

### Вариант 1: Запуск через systemd (рекомендуется)

```bash
# Проверьте статус сервиса
sudo systemctl status cloudcity

# Если сервис не настроен, создайте его:
sudo nano /etc/systemd/system/cloudcity.service
```

Вставьте:
```ini
[Unit]
Description=CloudCity ASP.NET Core App
After=network.target

[Service]
Type=notify
WorkingDirectory=/home/siteadmin/cloudcity/CloudCityCenter
ExecStart=/usr/bin/dotnet /home/siteadmin/cloudcity/CloudCityCenter/bin/Release/net8.0/CloudCityCenter.dll
Restart=always
RestartSec=10
KillSignal=SIGINT
SyslogIdentifier=cloudcity
Environment=ASPNETCORE_ENVIRONMENT=Production
Environment=ASPNETCORE_URLS=http://localhost:5000
Environment=USE_REVERSE_PROXY=true
User=siteadmin

[Install]
WantedBy=multi-user.target
```

Затем:
```bash
sudo systemctl daemon-reload
sudo systemctl enable cloudcity
sudo systemctl restart cloudcity
sudo systemctl status cloudcity
```

### Вариант 2: Запуск вручную (для тестирования)

**ВАЖНО:** Используйте `--no-launch-profile` чтобы не использовался Development профиль!

```bash
cd /home/siteadmin/cloudcity/CloudCityCenter

# Установите переменные окружения
export ASPNETCORE_ENVIRONMENT=Production
export ASPNETCORE_URLS="http://localhost:5000"
export USE_REVERSE_PROXY=true

# Запустите скомпилированное приложение (НЕ dotnet run!)
dotnet bin/Release/net8.0/CloudCityCenter.dll
```

Или если используете `dotnet run`:
```bash
dotnet run --configuration Release --no-launch-profile
```

### Вариант 3: Запуск в фоне

```bash
cd /home/siteadmin/cloudcity/CloudCityCenter

export ASPNETCORE_ENVIRONMENT=Production
export ASPNETCORE_URLS="http://localhost:5000"
export USE_REVERSE_PROXY=true

nohup dotnet bin/Release/net8.0/CloudCityCenter.dll > app.log 2>&1 &
```

## 🔍 Проверка

После запуска проверьте:

```bash
# 1. Проверьте порт
netstat -tlnp | grep 5000
# Должно показать: tcp 0 0 127.0.0.1:5000

# 2. Проверьте окружение в логах
# Должно быть: Hosting environment: Production (НЕ Development!)

# 3. Проверьте что приложение отвечает
curl http://localhost:5000
```

## ❌ Что НЕ делать

- ❌ НЕ используйте `dotnet run` без `--no-launch-profile`
- ❌ НЕ запускайте без установки `ASPNETCORE_ENVIRONMENT=Production`
- ❌ НЕ используйте порт 5076 (это Development порт)

## ✅ Правильный вывод

После правильного запуска вы должны увидеть:
```
info: Microsoft.Hosting.Lifetime[14]
      Now listening on: http://localhost:5000
info: Microsoft.Hosting.Lifetime[0]
      Application started. Press Ctrl+C to shut down.
info: Microsoft.Hosting.Lifetime[0]
      Hosting environment: Production  ← ВАЖНО!
info: Microsoft.Hosting.Lifetime[0]
      Content root path: /home/siteadmin/cloudcity/CloudCityCenter
```

