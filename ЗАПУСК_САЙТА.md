# 🚀 ЗАПУСК САЙТА - ПРОСТАЯ ИНСТРУКЦИЯ

## ⚡ БЫСТРЫЙ ЗАПУСК (3 команды)

Выполните эти команды на сервере **по порядку**:

```bash
cd /home/siteadmin/cloudcity
git pull
```

```bash
cd CloudCityCenter
dotnet build --configuration Release
```

```bash
export ASPNETCORE_ENVIRONMENT=Production
export ASPNETCORE_URLS="http://localhost:5000"
export USE_REVERSE_PROXY=true
nohup dotnet bin/Release/net8.0/CloudCityCenter.dll > app.log 2>&1 &
```

**Проверьте что работает:**
```bash
sleep 3
curl http://localhost:5000
```

Если видите HTML-код - приложение работает! Проверьте сайт: **https://cloudcitylife.com**

---

## 📋 ЕСЛИ САЙТ НЕ ОТКРЫВАЕТСЯ

### Шаг 1: Проверьте запущено ли приложение

```bash
ps aux | grep CloudCityCenter
```

Если процесса нет - запустите вручную (см. команды выше).

### Шаг 2: Проверьте слушает ли приложение порт

```bash
netstat -tlnp | grep 5000
# или
ss -tlnp | grep 5000
```

Если порт 5000 не слушается - приложение не запущено.

### Шаг 3: Проверьте Nginx

```bash
sudo systemctl status nginx
```

Если Nginx не запущен:
```bash
sudo systemctl start nginx
```

### Шаг 4: Проверьте конфигурацию Nginx

```bash
sudo nginx -t
```

Если есть ошибки - исправьте конфигурацию.

---

## 🔧 НАСТРОЙКА SYSTEMD СЕРВИСА (рекомендуется)

Чтобы приложение запускалось автоматически при перезагрузке сервера:

```bash
cd /home/siteadmin/cloudcity/CloudCityCenter
```

**Найдите путь к dotnet:**
```bash
which dotnet
```

**Создайте файл сервиса:**
```bash
sudo nano /etc/systemd/system/cloudcity.service
```

**Вставьте (ЗАМЕНИТЕ `/usr/bin/dotnet` на путь из `which dotnet`):**
```ini
[Unit]
Description=CloudCity ASP.NET Core App
After=network.target

[Service]
Type=notify
WorkingDirectory=/home/siteadmin/cloudcity/CloudCityCenter
ExecStart=/usr/bin/dotnet /home/siteadmin/cloudcity/CloudCityCenter/bin/Release/net8.0/CloudCityCenter.dll
Restart=always
RestartSec=10
KillSignal=SIGINT
SyslogIdentifier=cloudcity
Environment=ASPNETCORE_ENVIRONMENT=Production
Environment=ASPNETCORE_URLS=http://localhost:5000
Environment=USE_REVERSE_PROXY=true
User=siteadmin

[Install]
WantedBy=multi-user.target
```

**Активируйте и запустите:**
```bash
sudo systemctl daemon-reload
sudo systemctl enable cloudcity
sudo systemctl start cloudcity
sudo systemctl status cloudcity
```

---

## 📊 ПРОВЕРКА СТАТУСА

```bash
# Статус сервиса (если используете systemd)
sudo systemctl status cloudcity

# Процессы приложения
ps aux | grep dotnet

# Порты
netstat -tlnp | grep 5000

# Тест приложения
curl http://localhost:5000

# Тест через домен
curl https://cloudcitylife.com
```

---

## 🐛 ЛОГИ И ОШИБКИ

**Если используете systemd:**
```bash
sudo journalctl -u cloudcity -f
```

**Если запустили через nohup:**
```bash
tail -f /home/siteadmin/cloudcity/CloudCityCenter/app.log
```

**Логи Nginx:**
```bash
sudo tail -f /var/log/nginx/error.log
```

---

## ✅ КОНТРОЛЬНЫЙ СПИСОК

- [ ] Приложение собрано: `ls -la bin/Release/net8.0/CloudCityCenter.dll`
- [ ] Приложение запущено: `ps aux | grep CloudCityCenter`
- [ ] Порт 5000 слушается: `netstat -tlnp | grep 5000`
- [ ] Приложение отвечает: `curl http://localhost:5000`
- [ ] Nginx запущен: `sudo systemctl status nginx`
- [ ] Nginx конфигурация правильная: `sudo nginx -t`
- [ ] Сайт доступен через домен: `curl https://cloudcitylife.com`

---

## 🔄 ПЕРЕЗАПУСК

**Если используете systemd:**
```bash
sudo systemctl restart cloudcity
```

**Если запущено через nohup:**
```bash
# Найдите процесс
ps aux | grep CloudCityCenter
# Убейте его
kill <PID>
# Запустите заново (см. команды выше)
```
















